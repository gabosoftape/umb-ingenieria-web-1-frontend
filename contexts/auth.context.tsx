...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  // Cargar datos de autenticaciu00f3n del localStorage al iniciar
  useEffect(() => {
    const initAuth = () => {
      try {
        ...
        console.error("Error al inicializar la autenticaciu00f3n:", error);
        // Si hay un error, limpiar los datos de autenticaciu00f3n
        AuthService.logout();
      } finally {
        setLoading(false);
      }
  ...
  // Funciu00f3n para iniciar sesiu00f3n
  const login = async (email: string, password: string): Promise<AuthResponse> => {
    try {
      setLoading(true);
      const response = await AuthService.login({ email, password });
      
      // Guardar datos de autenticaciu00f3n
  ...
  // Funciu00f3n para registrar un nuevo usuario
  ...
  // Funciu00f3n para cerrar sesiu00f3n
  const logout = () => {
    AuthService.logout();
    setToken(null);
    setUser(null);
